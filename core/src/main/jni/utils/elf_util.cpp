...
ElfImg::ElfImg(const char *elf) {
    ...
    // this->elf = elf;
    //load elf
    int fd = open(this->elf, O_RDONLY);
    if (fd < 0) {
        LOGE("failed to open %s", this->elf);
        return;
    }

    size = lseek(fd, 0, SEEK_END);
    if (size <= 0) {
        LOGE("lseek() failed for %s", this->elf);
    }
...
#ifdef __LP64__
#define LOG_STR "find %s: %llx\n"
#else
#define LOG_STR "find %s: %x\n"
#endif
    Elf_Addr _offset = 0;
    //search dynmtab
